...
export class ScrollBarExec extends BashExec {
  protected cmd = "ember"
  ...
  // 根据 pushButtonExce 修改得到
  // 统一的 className 的作用？部分 attr 不需要
  ...
  private async addCompStyles(output: string, name: string, ) {
    ...
    // 伪元素
    this.component.css.filter((item) => item.pe !== "css").forEach((item) => {
      let pseudoEle: string = "." + this.component.name + "::" + item.pe + " {" + "\r" + "\n"
      const pseudoStyle = item.key + ": " + item.value + ";" + "\r"
  ...
  private recursiveComponents(component: BPComp): string {
    ...
    fileData = "{{#" + component.name + "}}" + "{{/" + component.name + "}}"
